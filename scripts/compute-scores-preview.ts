/**
 * compute-scores-preview.ts
 * Computes XP for all ambassadors from existing DB data (READ-ONLY — no writes).
 * Prints a ranked score table to stdout.
 *
 * Run: node --import tsx/esm scripts/compute-scores-preview.ts
 */
...
async function main() {
  ...
  if (!db) { console.error("DB not available — check DATABASE_URL"); process.exit(1); }
  ...
  for (const amb of ambassadors) {
    ...
    const c4 = decayScore(amb.c4ContentQuality, amb.c4UpdatedAt);
    const c5 = bulkC5(Number(tgData?.msgCount ?? 0));
    const c6 = decayScore(amb.c6CommunityValue, amb.c6UpdatedAt);
    const c7 = decayScore(amb.c7BuilderOutput, amb.c7UpdatedAt);
    const c8 = decayScore(amb.c8BuilderDepth, amb.c8UpdatedAt);
    const c9 = decayScore(amb.c9EngagementAuth, amb.c9UpdatedAt);
    const c10 = decayScore(amb.c10MissionAlign, amb.c10UpdatedAt);
    ...
    rows.push({
      handle: (amb.twitterHandle ?? amb.telegramHandle ?? `ID:${amb.id}`).replace(/^@/, ""),
  ...
  const colWidths = [3, 22, 4, 4, 5, 5, 4, 5, 5, 5, 5, 5, 4, 7, 7, 7];
  ...
  console.log(`Window: last ${ROLLING_WINDOW_DAYS} days (from ${windowStart.toISOString().slice(0,10)})`);
  console.log("NOTE: Scores NOT written to DB. Confirm before applying.");
  process.exit(0);
}
